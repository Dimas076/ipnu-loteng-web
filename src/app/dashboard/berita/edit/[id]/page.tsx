...
export default function EditBeritaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`/api/posts/${id}`, dataToSubmit);
  ...
  if (fetching) {
    return <div className="p-8 text-center text-on-surface-variant font-medium">Memuat data berita...</div>;
  }
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between bg-surface-container-lowest p-4 sm:p-6 rounded-lg border border-outline-variant">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight text-on-surface">Edit Berita</h1>
            <p className="text-sm text-on-surface-variant font-medium hidden sm:block">Ubah detail dan konten artikel Anda.</p>
          </div>
        </div>
